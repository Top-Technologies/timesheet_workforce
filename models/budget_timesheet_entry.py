...
class BudgetTimesheetEntry(models.Model):
    ...
    def create(self, vals_list):
        ...
        # After creation, if account_number was given, sync it to the contact
        for rec in records:
            if rec.account_number and rec.partner_id:
    ...
    def _sync_account_to_partner(self):
        """Create or update the partner's bank account to match the worker card."""
        self.ensure_one()
        ...
        if existing:
            # Update holder name if changed
            if self.account_holder_name and existing.acc_holder_name != self.account_holder_name:
                existing.acc_holder_name = self.account_holder_name
        else:
            # Check if the partner has ANY bank account — update number on first one,
            # or create a new one if they have none
            any_bank = PartnerBank.search([
                ('partner_id', '=', self.partner_id.id),
            ], limit=1)

            if any_bank:
                # Update the existing account's number to the new one
                any_bank.write({
                    'acc_number': self.account_number,
                    'acc_holder_name': self.account_holder_name or self.partner_id.name,
                })
            else:
                # Create a brand new bank account on the contact
                PartnerBank.create({
                    'partner_id': self.partner_id.id,
                    'acc_number': self.account_number,
                    'acc_holder_name': self.account_holder_name or self.partner_id.name,
